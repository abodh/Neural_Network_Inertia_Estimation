...
epoch = 8                 # number of epochs -> in 1 epoch all of the training data are used
mini_batch = 100             # number of mini-batches -> subset of the training data
...
frac_train = 0.70           # fraction of data to be used as training set
...
dropout_decision = True    # do you want to dropout or not?
...
#             "\\manipulated\\"
data_path = "..\\..\\matlab files\\0.2Hz\\manipulated\\"

# creating a unique folder to save the output files
str(date.today().strftime("%d/%m/%Y"))
output_path = "../../Neural-Network-Regression/log/" + str(date.today().strftime("%b-%d-%Y")) +\
...
train_loader = DataLoader(training, batch_size = mini_batch, shuffle = True, num_workers = 10, pin_memory = True )
validation_loader = DataLoader(validation, batch_size = mini_batch, shuffle = False, num_workers = 10, pin_memory = True)
...
optimizer = torch.optim.SGD(net.parameters(), lr = learning_rate, momentum = momentum, weight_decay = 0.01)
